...
lame_init_params(lame_global_flags *gfp) {
  ...
  if (cfg->vbr == vbr_off && gfp->compression_ratio > 0) {
    ...
    if (gfp->samplerate_out == 0)
      gfp->samplerate_out =
          map2MP3Frequency((int) (0.97 * gfp->samplerate_in)); /* round up with a margin of 3% */
  ...
  if (gfp->samplerate_out == 0 && (cfg->vbr == vbr_mt || cfg->vbr == vbr_mtrh)) {
    ...
    struct q_map const m[9]
        = {{48000, 0.0, 6.5, 0.0, 6.5, 23700}, {44100, 0.0, 6.5, 0.0, 6.5, 21780},
           {32000, 6.5, 8.0, 5.2, 6.5, 15800}, {24000, 8.0, 8.5, 5.2, 6.0, 11850},
           {22050, 8.5, 9.01, 5.2, 6.5, 10892}, {16000, 9.01, 9.4, 4.9, 6.5, 7903},
           {12000, 9.4, 9.6, 4.5, 6.0, 5928}, {11025, 9.6, 9.9, 5.1, 6.5, 5446},
...
lame_encode_buffer(lame_global_flags *gfp,
  ...
  return lame_encode_buffer_template(gfp,
                                     pcm_l,
                                     pcm_r,
                                     nsamples,
                                     mp3buf,
